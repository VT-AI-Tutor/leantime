...
class Reports
{
    ...
    public function runTicketReport(int $projectId, string $sprintId): array|bool
    {
        ...
        $query->selectRaw($this->dbHelper->wrapColumn('projectId'))
            ...
            ->selectRaw('SUM(CASE WHEN zp_tickets.storypoints = 4 THEN 1 ELSE 0 END) AS sum_todos_m')
            ->selectRaw('SUM(CASE WHEN zp_tickets.storypoints = 8 THEN 1 ELSE 0 END) AS sum_todos_l')
            ->selectRaw('SUM(CASE WHEN zp_tickets.storypoints = 12 THEN 1 ELSE 0 END) AS sum_todos_xl')
            ->selectRaw('SUM(CASE WHEN zp_tickets.storypoints = 16 THEN 1 ELSE 0 END) AS sum_todos_xxl')
